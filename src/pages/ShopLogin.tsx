  ...
  const handleSocialLogin = async (provider: 'google' | 'github') => {
    try {
      const { error: authError } = await supabase.auth.signInWithOAuth({
        provider,
        options: { redirectTo: `${window.location.origin}/${shopSlug}/compte` }
      });
          ...
          {!isForgotMode && (
            <>
              ...
              <div className="grid grid-cols-2 gap-3">
                <button type="button" onClick={() => handleSocialLogin('google')} className="bg-zinc-800 hover:bg-zinc-700 text-xs font-bold py-3.5 rounded-2xl">Google</button>
                <button type="button" onClick={() => handleSocialLogin('github')} className="bg-zinc-800 hover:bg-zinc-700 text-xs font-bold py-3.5 rounded-2xl">GitHub</button>
              </div>
            </>
          )}
